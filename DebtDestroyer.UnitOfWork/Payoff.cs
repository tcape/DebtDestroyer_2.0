...
namespace DebtDestroyer.UnitOfWork
{
    public class Payoff : IPayoff
    {
        ...
        public void SnowballSort()
        {
            _Accounts = _Accounts.ToList().OrderBy(account => account._Balance).ToList();
        }
        ...
        public bool CanPayAllMinimums()
        {
            foreach (var account in _Accounts)
            {
                if (account._Balance < account._MinPay)
                {
                    return false;
                }
            }
        ...
        public void PayAllMinimums()
        {
            foreach (var account in _Accounts)
            {
                if (account._Balance >= account._MinPay)
                {
                    account._Balance -= account._MinPay;
                    account._Payment += account._MinPay;
                }
            }
        }
        ...
        public void SnowballUpdate()
        {
            ...
            ApplyAllAccrued();
            SnowballSort(); // smallest balance to largest
            ResetPayments();

            var allocated = _AllocatedFunds;
            var totalPaid = 0.0m;

            // Pay all minimum payments that can be paid
            PayAllMinimums();
            ...
            foreach (var account in _Accounts)
            {
                // if account can be paid off, pay it off
                ...
                else // otherwise pay the rest of allocated
                {
        ...
        public IList<DebtDestroyer.Model.Payment> GenerateSnowball()
        {
